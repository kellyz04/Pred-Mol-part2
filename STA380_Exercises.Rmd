...
Step6:as we only care about the rent for the buildings that are similar to the one we are investing, the average rent for the buildings with 10-20 stories is higher for non-green versus green
```{r}
...
Now lets see which day has the worst. overall either one of the day there are more probablity of delay then on time. However the worst day to go is saturday. 
...
```
The average time of delays seems pretty even amount different days of the week
```{r}
...
```
If you can Aviod NW on Wednesday, it also shows that a lot of carriers tends to have longer delays over the weekends.
```{r}
...
```
As expected December have longer delays especailly on saturdays. Surprisingly, despite the fact that September to Novenmber has the mose delays, the average delay time was the shortest.
```{r}
...
```
Next time when you want to book for a flight, you can refer to these information to aviod delays or at least choose a time of shorter delay time.
...
```
Step3. Creat corpus for both data sets, and also make everything to lowercase. Remove numbers, remove punctuations, remove all white spaces, and remove all the stop words.
```{r}
...
```
Ste6. Our final step is to check for any missing words in the test sets using intersect function. 
```{r}
...
Step7. After getting our code ready, we decied to preform a random forest to predict for the correct author. We tried 100,500,1000 trees, and 50, 80, 100 for m. Using 500 tree and m = 80 give us the best result without too much compelxity. So our best model accuracy is about 80.48%.
```{r}
